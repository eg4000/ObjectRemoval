...
from GibsonEnv.gibson.core.physics.robot_locomotors import Quadrotor
...
class DroneNavigateEnv(CameraRobotEnv):
    ...
    """
    def __init__(self, config, gpu_idx=0, pano_type='rgb'):
        self.config = config #self.parse_config(config)
        print(self.config["envname"])
        assert(self.config["envname"] == self.__class__.__name__ or self.config["envname"] == "TestEnv")
        CameraRobotEnv.__init__(self, self.config, gpu_idx,
                                scene_type="building",
                                tracking_camera=tracking_camera, pano_type=pano_type)
